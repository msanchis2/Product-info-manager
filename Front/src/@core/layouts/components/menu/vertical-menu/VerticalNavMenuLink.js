...
import { fetchAndDownload, devuelveBasePath } from '@src/utility/Utils'
...
const VerticalNavMenuLink = ({
  ...
  //Obtengo los titulos, que son los controllerName
  // Monto Ability para controlar cual muestro dependiendo del titulo
  // Ability
  const ability = useAbility(AbilityContext)
  if (!ability.can(item.title, "PIM", "Ver")) {
    return null
  }
  ...
  return (
    ...
    >
      <LinkTag
        ...
        onClick={e => {
          if (!item.navLink.length) {
            e.preventDefault()
            if (item.downloadUrl) {
              const downloadFileName = item.downloadUrl.split("/").pop()
              if (downloadFileName) fetchAndDownload(`${devuelveBasePath()}${item.downloadUrl}`, downloadFileName)
            }
          }
      ...
      >
        {item.icon}
        {/* Pongo el titulo y se formatea dependiendo del item */}
