...
import { deletemarca, getData } from '../store/action'
    ...
    <div className='invoice-list-table-header w-100 py-2'>
      <Row>
        <Col lg='6' className='d-flex align-items-center px-0 px-lg-1'>
          ...
          <Can I="Marcas" a="PIM" field="Nuevo">
            <Button.Ripple tag={Link} to='/marcas/new' color='primary'>
...
const marcasList = () => {
  ...
  useEffect(() => {
    ...
    dispatch(
      getData(
        storeLayout.selectedEmpresaId,
        currentPage,
        rowsPerPage,
        searchTerm
      )
    )
  }, [searchTerm, currentPage, rowsPerPage, storeLayout.selectedEmpresaId])

  // controllerName, moduleName, Action
  if (!ability.can("Marcas", "PIM", "Ver")) {
    return null
  }

  // ** Modal Message
  const getDeleteMessage = () => {
    const selectedmarca = store.data.find(marca => marca.id === toDelete)
    return `¿Realmente desea eliminar el registro ${selectedmarca?.nombre}?`
  }

  // ** Modal Title
  const getDeleteTitle = () => {
    return intl.formatMessage({ id: 'Borrar marca' })
  }
