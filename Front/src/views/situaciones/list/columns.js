    ...
    },
    {
      ...
      cell: row => (
        <div className='column-action d-flex align-items-center'>
          <Link to={`/Situaciones/view/${row.id}`} id={`view-tooltip-${row.id}`}>
            <Eye size={17} className='mx-1' />
          </Link>
          <UncontrolledTooltip placement='top' target={`view-tooltip-${row.id}`}>
            <FormattedMessage id='Detalles' />
          </UncontrolledTooltip>
          <Can I="Situaciones" a="PIM" field="Actualizar">
            <Link to={`/Situaciones/edit/${row.id}`} id={`edit-tooltip-${row.id}`}>
          ...
          <Can I="Situaciones" a="PIM" field="Borrar">
