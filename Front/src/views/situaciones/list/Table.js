...
import { deleteSituacion, getData } from '../store/action'
    ...
    <div className='invoice-list-table-header w-100 py-2'>
      <Row>
        <Col lg='6' className='d-flex align-items-center px-0 px-lg-1'>
          ...
          <Can I="Situaciones" a="PIM" field="Nuevo">
            <Button.Ripple tag={Link} to='/Situaciones/new' color='primary'>
...
const SituacionesList = () => {
  ...
  if (!ability.can("Situaciones", "PIM", "Ver")) {
    return null
  }

  // ** Modal Message
  const getDeleteMessage = () => {
    const selectedSituacion = store.data.find(Situacion => Situacion.id === toDelete)
    return `¿Realmente desea eliminar el registro ${selectedSituacion?.nombre}?`
  }

  // ** Modal Title
  const getDeleteTitle = () => {
    return intl.formatMessage({ id: 'Borrar Situacion' })
  }
