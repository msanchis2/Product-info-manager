...
import { deleteIdioma, getData } from '../store/action'
    ...
    <div className='invoice-list-table-header w-100 py-2'>
      <Row>
        <Col lg='6' className='d-flex align-items-center px-0 px-lg-1'>
          ...
          <Can I="Idiomas" a="PIM" field="Nuevo">
            <Button.Ripple tag={Link} to='/idiomas/new' color='primary'>
...
const IdiomasList = () => {
  ...
  }, [searchTerm, currentPage, rowsPerPage, storeLayout.selectedEmpresaId, store.idiomas]) // Agregar aquí si se necesita refetchear según cambie alguna variable más,

  // controllerName, moduleName, Action
  if (!ability.can("Idiomas", "PIM", "Ver")) {
    return null
  }

  // ** Modal Message
  const getDeleteMessage = () => {
    const selectedIdioma = store.data.find(idioma => idioma.id === toDelete)
    return `¿Realmente desea eliminar el registro ${selectedIdioma?.nombre}?`
  }

  // ** Modal Title
  const getDeleteTitle = () => {
    return intl.formatMessage({ id: 'Borrar Idioma' })
  }
