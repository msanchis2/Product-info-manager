...
import { deleteUsuario, getData } from '../store/action'
    ...
    <div className='invoice-list-table-header w-100 py-2'>
      <Row>
        <Col lg='6' className='d-flex align-items-center px-0 px-lg-1'>
          ...
          <Can I="Usuarios" a="PIM" field="Nuevo">
            <Button.Ripple tag={Link} to='/usuarios/new' color='primary'>
        ...
        >
          <div className='d-flex align-items-center'>
            ...
            <Input
              ...
              placeholder={`${intl.formatMessage({ id: 'Usuario/Correo' })}`}
            />
          </div>
        </Col>
      </Row>
...
const UsuariosList = () => {
  ...
  const store = useSelector(state => state.usuarios)
  ...
  if (!ability.can("Usuarios", "PIM", "Ver")) {
    return null
  }

  // ** Modal Message
  const getDeleteMessage = () => {
    const selectedUsuario = store.data?.find(usuario => usuario.id === toDelete)
    return `¿Realmente desea eliminar el registro ${selectedUsuario?.nombre}?`
  }

  // ** Modal Title
  const getDeleteTitle = () => {
    return intl.formatMessage({ id: 'Borrar Usuario' })
  }
...
// Seteo a nulo, para que no coja el ultimo usuario cuando se vaya a crear uno nuevo
