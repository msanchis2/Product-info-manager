    ...
    },
    {
      ...
      cell: row => (
        <div className='column-action d-flex align-items-center'>
          <Link to={`/grupo-atributos/view/${row.id}`} id={`view-tooltip-${row.id}`}>
            <Eye size={17} className='mx-1' />
          </Link>
          <UncontrolledTooltip placement='top' target={`view-tooltip-${row.id}`}>
            <FormattedMessage id='Detalles' />
          </UncontrolledTooltip>
          <Can I="GrupoAtributos" a="PIM" field="Actualizar">
            <Link to={`/grupo-atributos/edit/${row.id}`} id={`edit-tooltip-${row.id}`}>
